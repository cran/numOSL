#####
decomp <-
function(Sigdata, ncomp=2, constant=TRUE, typ="cw", 
         control.args=list(), transf=TRUE, weight=FALSE, 
...
function(Sigdata, ncomp=2, constant=TRUE, typ="cw", 
         ...
         plot=TRUE, xylog=FALSE, lwd=3, outfile=NULL) {
    #### Stop if not.
    stopifnot(ncol(Sigdata)==2L, all(Sigdata[,2L,drop=TRUE]>0),
              length(ncomp)==1L, ncomp %in% seq(7L),
              length(constant)==1L, is.logical(constant),
              length(typ)==1L, is.character(typ), typ %in% c("cw","lm"),
    ...
    colnames(pars) <- c("Ithn", "Std.Ithn", "Lamda", "Std.Lamda")
    ...
    if (plot==TRUE) {
        ...
        plot(tim, sig, log=ifelse(xylog==TRUE,"xy","x"), las=0, cex.main=1.5, lab=c(7,7,9),
             ...
             pch=21, cex=ifelse(typ=="cw",1.5,1.25), bg="white", col="black")
        ...
        ###
        if (constant[1L]>0.0) {
            legend(ifelse(xylog==TRUE,"bottomleft",ifelse(typ=="cw","topright",ifelse(tim[which.max(sig)]>XaxisCentral,"topleft","topright"))),
                   legend=c("Fitted.Curve",paste("Comp.", seq(ncomp), sep=""),"Constant"), col=c("black",colors[seq(ncomp)],"black"),
                   pch=c(21,rep(NA,ncomp+1L)), lty=c(rep("solid",ncomp+1L),"dashed"),yjust=2,ncol=1, cex=1.5,bty="o",lwd=lwd, pt.bg="white")
        } else {
            legend(ifelse(xylog==TRUE,"bottomleft",ifelse(typ=="cw","topright",ifelse(tim[which.max(sig)]>XaxisCentral,"topleft","topright"))), 
                   legend=c("Fitted.Curve",paste("Comp.",seq(ncomp),sep="")), col=c("black",colors[seq(ncomp)]),
                   pch=c(21,rep(NA,ncomp)), lty=c(rep("solid",ncomp+1L)), yjust=2,ncol=1, cex=1.5,bty="o",lwd=lwd, pt.bg="white")
        } # end if.
        ### 
        grid(equilogs=FALSE)
        box(lwd=2L)
        ...
        plot(tim, sig-CompSig[,1L,drop=TRUE], log="x", 
             ...
             max(sig-CompSig[,1L,drop=TRUE])*1.05), xlab="Stimulation time (s)", 
             cex.lab=1.5, ylab="Residuals", xaxs="r", yaxs="i", type="p", 
             pch=21, cex=ifelse(typ=="cw",0.75,0.5), bg="black", col="gray")
        box(lwd=2L)
        ###
