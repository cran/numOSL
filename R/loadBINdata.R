...
function(filename, view=TRUE)  {
    ...
    repeat  {
        ...
        if ( Version %in% c(3L,4L) )  {             
              ...
              if (Version==6L)  {
                  ...
                  pass <- readBin(binFile, what="raw", n=33L, size=1L, endian="little")
              } else if (Version==8L)  {
    ...
    ###
    cat(paste("Successfully loaded ", length(records), " records\n\n", sep=""))
    ###
    if (view==TRUE) View(x=tab, title="Loaded Table of Records")
    ###
